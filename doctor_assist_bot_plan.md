...
- **Privacy first**: do not send patient-identifying information (PHI) to OpenManus, Gemini, Telegram, public URLs, Git, or general research storage.
- **Evidence before prose**: for medical facts, collect and preserve evidence before asking Gemini to write polished text.
...
- **Replaceable providers**: OpenManus, Gemini, PubMed, and future retrieval tools must be behind narrow interfaces so that one provider can be replaced without rewriting the UI.
...
|---|---|---|
| **Clinical** | Quick outpatient assistance | SOAP, patient explanation, differential diagnosis, drug question, free Gemini query |
...
## 6. Research Asset Design

### 6.1 Why an asset is needed
...
### 6.5 Minimum evidence quality policy
...
## 8. OpenManus Integration Boundary
...
├── modules/
│   ├── research_schemas.py           # Shared validated models
...
- Do not put PHI in `ResearchRequest`, OpenManus prompts, research JSON, Telegram messages, or logs.
...
- [ ] README/runbook explains how a new developer starts, tests, and troubleshoots it.

---

## 15. First Development Task

Do **not** start by editing the Vlog UI. Start with the reusable core:

1. Create `modules/research_schemas.py`.
...
Once this foundation is stable, OpenManus, Telegram commands, the Research tab, Vlog research checkbox, and patient-handout reuse become small integrations rather than separate systems.
